...
> 문서 상태: 스토리 모드 1차 구현 반영 v0.3
> 우선 개발 범위: 스토리 모드
> 후속 개발 범위: 배틀 모드, 실제 로그인
> 현재 기술 기반: 웹앱 + 브라우저 저장
> Supabase 원칙: 콘텐츠·스토리·배틀 검증 이후 마지막 연계 단계로 보류
> 구현 현황: 13개 에피소드, 94개 아이템, 로컬 진행도 저장, 서버 채점 완료
...
- 재도전과 다음 이야기 진행
- 게스트 진행 상황과 최고 점수 저장
- 코드 기반 시나리오·아이템 관리와 브라우저 최고 점수 저장
- 모바일, 태블릿, 데스크톱 대응
...
로그인 입력값은 MVP에서 저장하거나 외부 서비스로 전송하지 않는다.
...
스토리 모드 1차 버전은 프롤로그 1개와 4개 챕터의 12개 임무, 총 13개
에피소드로 구성한다.
...
Supabase Database·Storage·Edge Functions는 실제 로그인, 여러 기기 동기화,
랭킹 또는 배틀 기록이 필요해진 마지막 연계 단계에서만 검토한다.
...
- 콘텐츠 상태: 코드에서 읽는 챕터, 시나리오, 아이템
- 플레이 상태: 현재 선택한 옷, 남은 시간, 현재 단계
- 진행 상태: `localStorage`의 완료 여부, 최고 점수, 별 개수
...
| 단계 | 주요 결과물 | 예상 작업일 |
|---|---|---:|
| 0. 기반 설계 | 화면 흐름, 데이터 정의, 캐릭터 레이어 규격 | 완료 |
| 1. 아트 수직 시제품 | 문자 → 옷입히기 → 채점 → 결과와 레이어 에셋 | 완료 |
| 2. 아동 사용성 검증 | 5~8명 관찰, 이유 설명, 재도전 의향, 판정 | 현재 |
| 3. 검증 반영 | 치명·주요 문제 수정과 핵심 과업 재검증 | 2~5일 |
| 4. 스토리 시스템 | 스토리 맵, 잠금 해제, 브라우저 진행 저장 | 3~4일 |
| 5. 콘텐츠 확장 | 프롤로그 포함 13개 에피소드와 아이템 등록 | 4~6일 |
| 6. 배틀 모드 검증 | 공용 채점 엔진, 동점 규칙, 2인 흐름 | 별도 산정 |
| 7. 마지막 연계 | 실제 로그인과 Supabase 저장·동기화 | 별도 산정 |

Supabase 연계 전까지 모든 스토리 기능은 서버 데이터베이스 없이 완결되게
유지한다. 마지막 연계 단계에서도 로그인·동기화·랭킹·배틀 기록 중 실제로
필요한 데이터만 저장한다.
...
| 클라이언트 점수 조작 | 스토리에서는 학습 피드백을 우선하고, 경쟁 기능 도입 시 서버 검증 추가 |
| 기존 교육자료 이미지의 저작권 문제 | 자체 캐릭터와 자체 제작 자산만 제품에 사용 |
| 마지막 로그인 연계 시 기기 기록 손실 | 연계 직전에 `localStorage` 기록의 계정 병합 절차 설계 |
...
이 수직 시제품으로 옷 레이어, 조작감, 채점 공정성, 브라우저 저장,
모바일 화면을 먼저 검증한 뒤 나머지 에피소드를 확장한다. Supabase 연계는
스토리와 배틀 흐름의 제품 검증이 끝난 뒤 마지막에 진행한다.
